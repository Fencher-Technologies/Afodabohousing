-- ──────────────────────────────────────────────────────────────
-- 031: House Managers — dedicated table, FK anchors, auto-sync
-- ──────────────────────────────────────────────────────────────
...
-- House managers can read their own row
CREATE POLICY "House managers read own" ON house_managers
    FOR SELECT
    USING (user_id = auth.uid());

-- Tenants / others can read the manager of their property
...
COMMENT ON TABLE  house_managers                    IS 'Dedicated profile for house managers with FK anchors from properties/tenants';
COMMENT ON COLUMN house_managers.phone              IS 'Required — used for billing, SMS, and login';
COMMENT ON COLUMN house_managers.property_count     IS 'Denormalised count refreshed by triggers on properties.house_manager_id';
COMMENT ON COLUMN house_managers.tenant_count       IS 'Denormalised count refreshed by triggers on tenants.house_manager_id';
COMMENT ON COLUMN properties.house_manager_id       IS 'FK → house_managers(id). Read-only mirror of owner_id for business queries.';
COMMENT ON COLUMN tenants.house_manager_id          IS 'FK → house_managers(id). Read-only mirror of tenants.owner_id.';
