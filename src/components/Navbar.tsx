    ...
    <nav className="sticky top-0 z-50 bg-card/95 backdrop-blur-sm border-b border-border shadow-sm">
      <div className="container flex items-center justify-between h-16 px-4">
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-display font-bold text-xl text-primary hidden sm:block">Afodabohousing</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {role === 'admin' && (
            <Link to="/dashboard/admin" className="text-sm font-medium text-accent hover:text-accent/80 transition-colors flex items-center gap-1">
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            ...
            <>
              ...
              <Button size="sm" className="gradient-primary text-primary-foreground" onClick={() => navigate('/register')}>Get Started</Button>
            </>
          )}
        </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-card border-b border-border px-4 py-4 flex flex-col gap-3">
          <Link to="/properties" className="text-sm font-medium py-2" onClick={() => setMenuOpen(false)}>Browse Properties</Link>
          {user ? (
            <>
              <Link to={dashboardPath} className="text-sm font-medium py-2" onClick={() => setMenuOpen(false)}>{dashboardLabel}</Link>
            ...
            <>
              ...
              <Button size="sm" className="gradient-primary text-primary-foreground" onClick={() => { navigate('/register'); setMenuOpen(false); }}>Get Started</Button>
            </>
          )}
        </div>
      )}
